...
#' @param add.annotations Add annotations in format of W3C WebAnnotation model. Defaults to \code{NULL}.
...
annotorious <- function(inputId = "annotations",
  ...
  if(type %in% "annotorious"){
    htmlwidgets::createWidget(name = 'annotorious', x, width = width, height = height, package = 'irecogito', elementId = elementId, dependencies = dependencies)
  }else if(type %in% "openseadragon"){
    htmlwidgets::createWidget(name = 'annotoriousopenseadragon', x, width = width, height = height, package = 'irecogito', elementId = elementId, dependencies = dependencies)
  }else if(type %in% "openseadragon-notoolbar"){
    htmlwidgets::createWidget(name = 'annotoriousopenseadragonnotoolbar', x, width = width, height = height, package = 'irecogito', elementId = elementId, dependencies = dependencies)
  }
}
...
#' library(shiny)
#' library(irecogito)
#' url <- system.file(package = "irecogito", "examples", "Pamphlet_dutch_tulipomania_1637.jpg")
...
#'               sep = "")
#' \dontshow{
#' urls <- file.path(system.file(package = "irecogito", "examples"),
...
annotoriousOutput <- function(outputId, width = '100%', height = '400px'){
  htmlwidgets::shinyWidgetOutput(outputId, 'annotorious', width, height, package = 'irecogito')
}
...
openseadragonOutput <- function(outputId, width = '100%', height = '400px'){
  htmlwidgets::shinyWidgetOutput(outputId, 'annotoriousopenseadragon', width, height, package = 'irecogito')
}
...
openseadragonOutputNoToolbar <- function(outputId, width = '100%', height = '400px'){
  htmlwidgets::shinyWidgetOutput(outputId, 'annotoriousopenseadragonnotoolbar', width, height, package = 'irecogito')
}
...
read_annotorious <- function(x, src = character()){
  default <- data.frame(id = character(), type = character(), label = I(list()), comment = I(list()), x = numeric(), y = numeric(), width = numeric(), height = numeric(), r = numeric(), polygon = I(list()), stringsAsFactors = FALSE)
  if(is.character(x)){
    ...
    if(length(x) == 0){
      ...
      x <- data.frame(id = sapply(x, FUN = function(x) x$id),
                      type = sapply(x, FUN = function(x){
                        ...
                        ifelse(grepl(x, pattern = "xywh=pixel"), "RECTANGLE",
                               ifelse(grepl(x, pattern = "polygon points"), "POLYGON", 
                                      ifelse(grepl(x, pattern = "circle"), "CIRCLE", NA)))
                      }),
                      label = I(tags),
                      comment = I(comment),
      ...
      # change type to point if width = height = 0 for all such cases
      if(length(idx) > 0){
        for (i in 1:length(idx)) {
          if (x$width[idx][i] == 0 & x$height[idx][i] == 0) {
            x$type[idx][i] = "POINT"
            x$width[idx][i] = NA
            x$height[idx][i] = NA
          }
      ...
      idx <- which(x$type == "CIRCLE") 
      x$x[idx] = sapply(strsplit(x$content[idx], split = '"'), FUN = function(x) x[2])
      x$y[idx] = sapply(strsplit(x$content[idx], split = '"'), FUN = function(x) x[4])
      x$r[idx] = sapply(strsplit(x$content[idx], split = '"'), FUN = function(x) x[6])
      ...
      out <- x[, c("id", "type", "label", "comment", "x", "y", "width", "height", "r", "polygon"), drop = FALSE]
  ...
  out <- out[, c("id", "type", "label", "comment", "x", "y", "width", "height", "r", "polygon"), drop = FALSE]
  attr(out, "src") <- src
  out
}
